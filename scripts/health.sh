...
airflow_status=$(docker compose ps --format json airflow | python3 -c 'import json,sys; rows=[json.loads(line) for line in sys.stdin if line.strip()]; print(rows[0].get("Health", "") if rows else "missing")')
if [[ "$airflow_status" != "healthy" ]]; then
  echo "Airflow standalone service is not healthy: $airflow_status" >&2
  exit 1
fi

echo "Airflow standalone service: healthy"
